...
// Function to update star states and ranks with debouncing
let updateStarStatesDebounce = null;
function updateStarStates(clickCount = 1) {
    // Clear any existing debounce
    if (updateStarStatesDebounce) {
        clearTimeout(updateStarStatesDebounce);
    }

    updateStarStatesDebounce = setTimeout(() => {
        ...
        for (let i = 1; i <= 16; i++) {
            ...
            if (bottomStar) {
                const currentState = parseInt(progress[`exercise1:1:${i}`] || "0"); // Example mapping, adjust as needed
                let newState = Math.min(Math.max(currentState + clickCount, 0), 6); // Limit to 0-6
                progress[`exercise1:1:${i}`] = newState.toString(); // Update progress
                if (i <= sixStarCount) {
                    bottomStar.setAttribute("fill", "#ffd700");
                    bottomStar.setAttribute("stroke", "#000000");
                    bottomStar.setAttribute("stroke-width", "1");
                    console.log(`bottom_star${i} turned gold (state: ${newState})`);
                } else {
                    bottomStar.setAttribute("fill", "#000000");
                    bottomStar.removeAttribute("stroke");
                    bottomStar.removeAttribute("stroke-width");
                    console.log(`bottom_star${i} turned black (state: ${newState})`);
                }
            } else {
    ...
    }, 200); // Debounce delay of 200ms to handle rapid clicks
}
