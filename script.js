document.addEventListener('DOMContentLoaded', () => {
    ...
    // Name prompt logic (only applies on index.html)
    ...
    userNameDisplay.style.fontSize = '0.9vw';
    userNameDisplay.style.textAlign = 'center';
    userNameDisplay.style.marginTop = '10px';
    document.querySelector('.rank-badge')?.appendChild(userNameDisplay);

    function showPopup() {
        if (namePopup) {
            namePopup.style.display = 'flex';
        } else {
            console.log('namePopup element not found (expected on chapter pages)');
        }
    }
    ...
    // Attach event listeners for name prompt
    if (submitBtn) {
        submitBtn.addEventListener('click', saveName);
    } else {
        console.log('submitNameBtn not found (expected on chapter pages)');
    }
    if (cancelBtn) {
        cancelBtn.addEventListener('click', closePopup);
    } else {
        console.log('cancelNameBtn not found (expected on chapter pages)');
    }
    ...
    // Prevent default form submission if added later
    document.querySelectorAll('#namePopup button').forEach(button => {
