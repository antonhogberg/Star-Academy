...
const translations = {
    ...
    sv: {
        ...
        faqA4: "Ja, i Hantera elever: skriv elevens namn, tryck Lägg till, skifta mellan elever när du vill—data sparas lokalt ochares automatiskt.",
...
function retryLoadStarMap(element, retries = 3, delay = 1000) {
    if (retries <= 0) {
        console.error('Failed to load Star Map SVG after retries');
        return;
    }
    console.warn(`Retrying Star Map SVG load... (${retries} retries left)`);
    setTimeout(() => {
        const src = element.getAttribute('data');
        element.setAttribute('data', ''); // Clear the src
        element.setAttribute('data', src); // Reset the src to trigger reload
        retries--;
        // Kontrollera om laddningen lyckades efter nästa försök
        setTimeout(() => {
            if (!element.contentDocument) {
                retryLoadStarMap(element, retries, delay);
            }
        }, delay);
    }, delay);
}
...
waitForDOM().then(() => {
    ...
    // Handle starmap.html loading issues and SVG loading
    if (window.location.pathname.toLowerCase().includes('starmap.html')) {
        ...
        // Handle SVG loading
        const starMapObject = document.getElementById('starMap');
        if (starMapObject) {
            starMapObject.addEventListener('load', () => {
                console.log('Star Map SVG loaded successfully');
                if (typeof window.initializeStarMap === 'function' && studentsData.currentStudent) {
                    window.initializeStarMap();
                }
            });
            starMapObject.addEventListener('error', () => {
                console.error('Error loading Star Map SVG');
                retryLoadStarMap(starMapObject); // Retry loading SVG
            });
            // Check if SVG loaded after a timeout
            setTimeout(() => {
                if (!starMapObject.contentDocument) {
                    console.warn('Star Map SVG not loaded after 2 seconds, retrying...');
                    retryLoadStarMap(starMapObject);
                }
            }, 2000);
        } else {
            console.error('Star Map object not found');
        }
    }
        ...
        if (starMapContainer && titleContainer && body) {
            ...
            if (starMapSvg) {
                if (isMobile) {
                    ...
                    starMapSvg.style.objectFit = 'none';
                }
            }
